...
`inject.py` is a Python command line application with the following usage:
```
inject.py \
...
- `--camera` Camera index (integer) to use. Change this to switch between real and virtual camera. 
...
python ../../../samples/python/inject-nostandard/inject.py --assets ../../../assets --camera 0
```
...
endlocal
```
If you want to make your life easier run [python_inject.bat](../../../binaries/windows/x86_64/python_inject.bat) to test on Windows. You can edit the file using Notepad to change the parameters.

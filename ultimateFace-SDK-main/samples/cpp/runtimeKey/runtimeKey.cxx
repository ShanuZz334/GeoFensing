...
int main(int argc, char *argv[])
{
	...
	ULTFACE_SDK_ASSERT(UltFaceSdkEngine::init(jsonConfig.c_str()).isOK());

	// Request runtime license key
	const UltFaceSdkResult result = UltFaceSdkEngine::requestRuntimeLicenseKey(rawInsteadOfJSON);
	if (result.isOK()) {
	...
	ULTFACE_SDK_ASSERT(UltFaceSdkEngine::deInit().isOK());
...
{
	if (!message.empty()) {
		ULTFACE_SDK_PRINT_ERROR("%s", message.c_str());
	}
